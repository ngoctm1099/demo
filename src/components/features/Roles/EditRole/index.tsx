...
import { moduleNames, roleDefaultValues, RoleInputs } from "../../../../constant/defaultValues";
import { roleMapping } from "../../../../constant/defaultValues";
...
const EditRoleForm = ({ open, selectedRole, setOpen }: EditRoleFormProps) => {
  ...
  useEffect(() => {
    if (selectedRole) {
      setValue("role", selectedRole?.role);
      Object.values(roleMapping).forEach(({ label, roleActions }, moduleIndex) => {
        roleActions?.forEach((role: keyof RoleInputs, roleIndex) => {
          const roleValue = selectedRole.roleActions[moduleIndex];
          setValue(role, roleValue ? (roleValue[roleIndex] === "1" ? true : false) : false);
        });
        const isCheckedAll = roleActions?.map((role: keyof RoleInputs) => watch(role))?.every(role => role);
        setValue(label as keyof RoleInputs, isCheckedAll);
      });
    }
  }, [selectedRole]);
  ...
  const handleCheckboxAfterClick = (label, roleActions) => {
    const isSelectedAll = roleActions.every(role => watch(role));
  ...
  const handleUpdatingRole = async () => {
    ...
    return await put("/roles", {
      role: {
        role: selectedRole?.role,
      },
      newRole: { role: selectedRole?.role, moduleNames, roleValues },
    })
      .then(res => {
  ...
  const roleSelectionTable = (
    <div className="text-xs sm:text-sm">
      ...
      <Button
        ...
        title={toLocales("fe_role_mid_popup_update")}
        onClick={handleSubmit(handleUpdatingRole)}
      />
    </div>
  );
  ...
  return (
    <Modal
      open={open}
      setOpen={setOpen}
      header={`${toLocales("fe_role_mid_popup_edit")} ${selectedRole?.role}`}
      className="!max-w-3xl"
